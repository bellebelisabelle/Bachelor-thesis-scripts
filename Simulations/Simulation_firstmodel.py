...
for theta in thetas:
    ...
    ax1.plot(-1,-1,'.',color='tab:green',label=r'$\phi=\frac{\pi}{4}$')
    ...
    ax2.plot(-1,-1,'.',color='tab:red',label=r'$\phi=\frac{\pi}{4}$')
    ...
    ax2.set_yticks([0,np.pi/2,np.pi,3*np.pi/2, 2*np.pi],['0',r'$\frac{\pi}{2}$',r'$\pi$',r'$\frac{3\pi}{2}$',r'$2\pi$'])
    ax2.legend()
    plt.savefig(f"Figures/Convergence_p{p}_h{h}_theta{theta}.pdf",dpi=300,bbox_inches='tight')
    plt.show()
