...
def F3(r,theta,h):
    w = h**2+theta**2*r**2
    F3 = F2(r,theta,h)/2 + theta*r/2*g(w)
    return F3
    
def F3der(r,theta,h):
    w = h**2+theta**2*r**2
    wder = 2*theta**2*r
    F3der = 1/2 * F2der(r,theta,h) + theta/2*g(w) + theta*r/2*wder*gder(w)
    return F3der
...
def F3der2(r,theta,h):
    ...
    wder = 2*theta**2*r
    wder2 = 2*theta**2
    F3der2 = 1/2 * F2der2(r,theta,h) + theta*wder*gder(w) \
...
def F4(r,theta,h):
    ...
    F4 = theta*r/2 * (g(c_pos)+g(c_neg))+h*np.sqrt(2)/4*(g(c_pos)-g(c_neg))
    return F4
